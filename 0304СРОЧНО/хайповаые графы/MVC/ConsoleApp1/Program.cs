...
public class Graph
{
    ...
    public List<List<int>> StronglyConnectedVerticles() =>
        TarjanAlgorithm.FindStronglyConnectedComponents(this);
    ...
    internal static class TarjanAlgorithm
    {
        private static int index = 0;
        private static Stack<int> stack = new();
        private static List<List<int>> strongComponents = [];
        private static bool[] onStack;
        private static int[] indexArr, lowlink;

        public static List<List<int>> FindStronglyConnectedComponents(Graph graph)
        {
            onStack = new bool[graph.VertexesCount];
            indexArr = new int[graph.VertexesCount];
            lowlink = new int[graph.VertexesCount];
        ...
        private static void StrongConnect(Graph graph, int v)
        {
            indexArr[v] = index;
            lowlink[v] = index;
            index++;
            ...
            for (int w = 0; w < graph.VertexesCount; w++)
            {
                if (graph.IsAdjastent(new(v), new(w)))
                {
                    if (indexArr[w] == 0)
                    {
...
public class Program
{
    public static void Main()
    {
        ...
        int n = 9;
        int m = 5;
        (int, int)[] roads = [(0, 3), (3, 4), (4, 6), (2, 8), (2, 5)];
        for(int i = 0; i < n; i++)
        {
            graph.AddVertex(new(i));
        }
        foreach(var (from , to) in roads)
        {
            graph.AddEdge(new(from), new(to), 1);
            graph.AddEdge(new(to), new(from), 1);
        }

        var components = graph.StronglyConnectedVerticles();

        Console.WriteLine("Количество дорог: " + (components.Count - 1));
        var neccesaryRoads = components.Select(comp => comp.First()).ToArray();
        for(int i = 0; i < neccesaryRoads.Length - 1; i++)
        {
            Console.WriteLine($"Можно построить дорогу между {neccesaryRoads[i]} и {neccesaryRoads[i+1]}") ;
        }
